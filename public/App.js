...
<div class="${"row"}">
  ${each(loadList(), item => `<div class="${"card m-3 col-md-5 svelte-1ghgllr"}">
      <div class="${"row no-gutters"}">
        <div class="${"col-md-4 col-sm-3"}">
          <img src="${"https://via.placeholder.com/150"}" class="${"card-img"}" alt="${""}">
        </div>
        <div class="${"col-md-8 col-sm-9"}">
          <div class="${"card-body"}">
            <h5 class="${"card-title"}">${escape(item.name)}</h5>
            <p class="${"lead"}">Episode: ${escape(item.current)}/${escape(item.total)}</p>
            
            <div class="${"btn-group"}" role="${"group"}" aria-label="${"episode counter"}">
              <button type="${"button"}" class="${"btn btn-lg btn-warning"}">-</button>
              <button type="${"button"}" class="${"btn btn-lg btn-success"}">+</button>
            </div>
            <p class="${"card-text"}"><small class="${"text-muted"}">Last updated on: ${escape(item.lastUpdated)}</small></p>
          </div>
        </div>
      </div>
    </div>`)}
...
const App = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	...
	return `${($$result.head += `<link rel="${"stylesheet"}" href="${"https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css"}"><script src="${"https://code.jquery.com/jquery-3.4.1.slim.min.js"}" integrity="${"sha384-J6qa4849blE2+poT4WnyKhv5vZF5SrPo0iEjwBvKU7imGFAV0wwj1yYfoRSJoZ+n"}" crossorigin="${"anonymous"}"></script><script src="${"https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js"}" integrity="${"sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo"}" crossorigin="${"anonymous"}"></script><script src="${"https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js"}" integrity="${"sha384-wfSDF2E50Y2D1uUdj0O3uMBJnjuUD4Ih7YwaYd1iqfktj0Uod8GCExl3Og8ifwB6"}" crossorigin="${"anonymous"}"></script>`, "")}
...
${validate_component(Router, "Router").$$render($$result, { url }, {}, {
	...
	<nav class="${"navbar navbar-expand-lg navbar-dark bg-dark"}">
		...
		<div class="${"collapse navbar-collapse"}" id="${"navbarNav"}">
			<ul class="${"navbar-nav"}">
				<li class="${"nav-item"}">
					${validate_component(NavLink, "Link").$$render($$result, { to: "/", class: "active" }, {}, { default: () => `Home` })}
				</li>
				<li class="${"nav-item"}">
					${validate_component(NavLink, "Link").$$render($$result, { to: "about" }, {}, { default: () => `About` })}
				</li>
			</ul>
		</div>
	</nav>
